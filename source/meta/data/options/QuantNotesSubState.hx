...
class QuantNotesSubState extends MusicBeatSubstate
{
	...
	public function new()
	{
		...
		{
			blackBG = new FlxSprite(posX - 25).makeGraphic(870, 200, FlxColor.WHITE);
			blackBG.alpha = 0.4;
			var actualBG:FlxSprite = new FlxSprite().makeGraphic(1280, 720, FlxColor.BLACK);
			actualBG.alpha = 0.5;
			add(actualBG);
		}
		add(blackBG);
		...
		for (i in 0...ClientPrefs.data.quantHSV.length)
		{
			var yPos:Float = (165 * i) + 35;
			for (j in 0...3)
			{
				var optionText:Alphabet = new Alphabet(0, yPos + 60, Std.string(ClientPrefs.data.quantHSV[i][j]), true);
			...
			newShader.hue = ClientPrefs.data.quantHSV[i][0] / 360;
			newShader.saturation = ClientPrefs.data.quantHSV[i][1] / 100;
			newShader.lightness = ClientPrefs.data.quantHSV[i][2] / 100;
		...
		changeSelection();
		if (meta.states.desktop.DesktopOptionsState.instance != null)
			cameras = [meta.states.desktop.DesktopOptionsState.instance.camCRT];
	}
	...
	override function update(elapsed:Float)
	{
		...
		if (controls.BACK || (changingNote && controls.ACCEPT))
		{
			if (!changingNote)
			{
				...
				#end
				if (!ProgressionHandler.isRodent)
					meta.states.desktop.DesktopOptionsState.monitor.visible = true;
			}
			else
			{
				changeSelection();
	...
	function changeSelection(change:Int = 0)
	{
		curSelected += change;
		if (curSelected < 0)
			curSelected = ClientPrefs.data.quantHSV.length - 1;
		if (curSelected >= ClientPrefs.data.quantHSV.length)
			curSelected = 0;

		curValue = ClientPrefs.data.quantHSV[curSelected][typeSelected];
		updateValue();
	...
	function resetValue(selected:Int, type:Int)
	{
		...
		{
			ClientPrefs.data.quantHSV[selected][type] = defaults[selected][type];
			switch (type)
			{
				case 0:
		...
		else if (ClientPrefs.data.noteSkin == "QuantStep")
		{
			ClientPrefs.data.quantHSV[selected][type] = quantStepmania[selected][type];
			switch (type)
			{
				case 0:
	...
	function updateValue(change:Float = 0)
	{
		...
		ClientPrefs.data.quantHSV[curSelected][typeSelected] = roundedValue;
