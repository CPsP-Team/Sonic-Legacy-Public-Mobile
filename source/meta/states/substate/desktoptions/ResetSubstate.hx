...
class ResetSubstate extends MusicBeatSubstate
{
	...
	public static function destroyData(deleteProgress:Bool, deleteSave:Bool = false)
	{
		...
		if (deleteSave)
		{
			for (i in Reflect.fields(ClientPrefs.defaultData))
				Reflect.setField(ClientPrefs.data, i, Reflect.field(ClientPrefs.defaultData, i));
		}
