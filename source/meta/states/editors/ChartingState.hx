...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Camera Flash', "Value1: camera\n (if empty will use the game camera.)\nValue2: duration,color\n (if empty, duration will be 0.5 and color will be white.)"],
		['Camera Zoom', "Changes the Camera Zoom.\n\nValue 1: Zoom Multiplier (1 is default)\n\nIn case you want a tween, use Value 2 like this:\n\n\"3, elasticOut\"\n(Duration, Ease Type)"],
		['subtitles','val1: text,time'],
		['Change Focus','switches who the camera is focused on.\nValue1: dad,bf,gf (bf is default)'],
		['tweenCamZoom'],
		['changeUI','value1: new ui class name'],
		['2Torial'],
		['Obituary']
	];
	...
	**/
	...
	override function create()
	{
		...
		DiscordHandler.changePresence("Chart Editor", "Uhm idk mane burp");
		...
		gradient = FlxGradient.createGradientFlxSprite(FlxG.width, FlxG.height * 8, [0x0, ClientPrefs.data.editorGradColors[0], ClientPrefs.data.editorGradColors[1], 0x0]);
		// gradient.setPosition(0, ((FlxG.height * 4) * -1));
		gradient.scrollFactor.set(0, 0);
		gradient.updateHitbox();
		gradient.visible = ClientPrefs.data.editorGradVis;
		// gradient.alpha = 1;
		add(gradient);
		...
		UI_box.color = ClientPrefs.data.editorUIColor;
		...
		textBox = new FlxSprite().makeGraphic(Std.int(clickForInfo.width * 1.25), Std.int(clickForInfo.height * 1.25), FlxColor.fromRGB(ClientPrefs.data.editorUIColor.red, ClientPrefs.data.editorUIColor.green, ClientPrefs.data.editorUIColor.blue));
	...
	function addVisualsUI():Void
	{
		...
		var gradient1colors = new FlxUIInputText(10, 30, 150, '${ClientPrefs.data.editorGradColors[0].red}, ${ClientPrefs.data.editorGradColors[0].green}, ${ClientPrefs.data.editorGradColors[0].blue}', 8);
		var gradient2colors = new FlxUIInputText(10, 50, 150, '${ClientPrefs.data.editorGradColors[1].red}, ${ClientPrefs.data.editorGradColors[1].green}, ${ClientPrefs.data.editorGradColors[1].blue}', 8);
		...
		var changecolors:FlxButton = new FlxButton(180, 37.5, "Change colors", function()
		{
			...
			ClientPrefs.data.editorGradColors[0] = FlxColor.fromRGB(grad1Colors[0], grad1Colors[1], grad1Colors[2]);
			ClientPrefs.data.editorGradColors[1] = FlxColor.fromRGB(grad2Colors[0], grad2Colors[1], grad2Colors[2]);
		...
		check_grad_vis.callback = function(){
			...
			ClientPrefs.data.editorGradVis = gradient.visible;
			ClientPrefs.saveSettings();
		}
		...
		var boxTxtColors1 = new FlxUIInputText(10, 115, 150, '${ClientPrefs.data.editorBoxColors[0].red}, ${ClientPrefs.data.editorBoxColors[0].green}, ${ClientPrefs.data.editorBoxColors[0].blue}', 8);
		var boxTxtColors2 = new FlxUIInputText(10, 135, 150, '${ClientPrefs.data.editorBoxColors[1].red}, ${ClientPrefs.data.editorBoxColors[1].green}, ${ClientPrefs.data.editorBoxColors[1].blue}', 8);
		...
		var changecolors:FlxButton = new FlxButton(180, 125, "Change colors", function()
		{
			...
			ClientPrefs.data.editorBoxColors[0] = FlxColor.fromRGB(box1Colors[0], box1Colors[1], box1Colors[2]);
			ClientPrefs.data.editorBoxColors[1] = FlxColor.fromRGB(box2Colors[0], box2Colors[1], box2Colors[2]);
			ClientPrefs.saveSettings();
		...
		var uiBoxTxt = new FlxUIInputText(10, 175, 150, '${ClientPrefs.data.editorUIColor.red}, ${ClientPrefs.data.editorUIColor.green}, ${ClientPrefs.data.editorUIColor.blue}', 8);

		var changecolors:FlxButton = new FlxButton(180, 170, "Change Color", function()
		{
			var shit = uiBoxTxt.text.split(', ');

			ClientPrefs.data.editorUIColor = FlxColor.fromRGB(Std.parseInt(shit[0]), Std.parseInt(shit[1]), Std.parseInt(shit[2]));
			ClientPrefs.saveSettings();
		...
		var prsList = new FlxUIDropDownMenu(10, 270, FlxUIDropDownMenu.makeStrIdLabelArray(ClientPrefs.data.chartPresetList), function(preset:String){
			var presetToUse = ClientPrefs.data.chartPresets.get(preset);
			ClientPrefs.data.editorGradColors = presetToUse[0];
			ClientPrefs.data.editorGradVis = presetToUse[1];
			ClientPrefs.data.editorBoxColors = presetToUse[2];
			ClientPrefs.data.editorUIColor = presetToUse[3];
			ClientPrefs.saveSettings();

			gradient.visible = ClientPrefs.data.editorGradVis;
			check_grad_vis.checked = ClientPrefs.data.editorGradVis;
			UI_box.color = ClientPrefs.data.editorUIColor;
			reloadGridLayer();
			resetIdle();
		});
		...
		var newPrsButton = new FlxButton((newPrsName.x + newPrsName.width) + 10, 240, "New Preset", function(){
			if(!ClientPrefs.data.chartPresets.exists(newPrsName.text))
				ClientPrefs.data.chartPresetList.push(newPrsName.text);
			ClientPrefs.data.chartPresets.set(newPrsName.text, [[ClientPrefs.data.editorGradColors[0], ClientPrefs.data.editorGradColors[1]], false, [ClientPrefs.data.editorBoxColors[0], ClientPrefs.data.editorBoxColors[1]], ClientPrefs.data.editorUIColor]);
			ClientPrefs.saveSettings();

			prsList.setData(FlxUIDropDownMenu.makeStrIdLabelArray(ClientPrefs.data.chartPresetList));

			trace('New Preset! [${newPrsName.text}]\nValue: ${ClientPrefs.data.chartPresets.get(newPrsName.text)}');	
		});
		...
		var clearPresets = new FlxButton((prsList.x + prsList.width) + 10, 270, "Clear Presets", function(){
			openSubState(new Prompt('This action will clear all presets.\n\nProceed?', 0, function(){
				ClientPrefs.data.chartPresets.clear();
				ClientPrefs.data.chartPresetList = ['Default'];
				ClientPrefs.data.chartPresets.set('Default', [[FlxColor.fromRGB(0,0,0), FlxColor.fromRGB(0,0,0)], false, [FlxColor.fromRGB(255,255,255), FlxColor.fromRGB(210, 210, 210)], FlxColor.fromRGB(250,250,250)]);
				ClientPrefs.saveSettings();
	...
	override function update(elapsed:Float)
	{
		...
		if(FlxG.mouse.overlaps(clickForInfo)){
			...
			if(FlxG.mouse.justPressed){
				...
				FlxTween.tween(clickForInfo, {alpha: 0}, 0.45);

				FlxTween.tween(textBox, {x: 0, y: 0, alpha: 0.6}, 0.45, {ease: FlxEase.quartOut});
				FlxTween.tween(textBox.scale, {x: 8.25806451613, y: 34.2857142857}, 0.45, {ease: FlxEase.quartOut, onComplete: function(shit:FlxTween){
	...
	*/
	...
	function reloadGridLayer() {
		gridLayer.clear();
		gridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats() * 4 * zoomList[curZoom]), true, ClientPrefs.data.editorBoxColors[0], ClientPrefs.data.editorBoxColors[1]);
		#if desktop
		...
		{
			prevGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats(curSec - 1) * 4 * zoomList[curZoom]), true, FlxColor.fromRGB(ClientPrefs.data.editorBoxColors[0].red - 40, ClientPrefs.data.editorBoxColors[0].green - 40, ClientPrefs.data.editorBoxColors[0].blue - 40), FlxColor.fromRGB(ClientPrefs.data.editorBoxColors[1].red - 40, ClientPrefs.data.editorBoxColors[1].green - 40, ClientPrefs.data.editorBoxColors[1].blue - 40));
		...
		{
			nextGridBG = FlxGridOverlay.create(GRID_SIZE, GRID_SIZE, GRID_SIZE * 9, Std.int(GRID_SIZE * getSectionBeats(curSec + 1) * 4 * zoomList[curZoom]), true, FlxColor.fromRGB(ClientPrefs.data.editorBoxColors[0].red - 40, ClientPrefs.data.editorBoxColors[0].green - 40, ClientPrefs.data.editorBoxColors[0].blue - 40), FlxColor.fromRGB(ClientPrefs.data.editorBoxColors[1].red - 40, ClientPrefs.data.editorBoxColors[1].green - 40, ClientPrefs.data.editorBoxColors[1].blue - 40));
	...
	function resetIdle(){
		lilOpp.color = ClientPrefs.data.editorBoxColors[0];
		lilBf.color = ClientPrefs.data.editorBoxColors[0];
		lilStage.color = ClientPrefs.data.editorBoxColors[1];
		lilBf.animation.play("idle");
		lilOpp.animation.play("idle");
	}
}
...
class ChartingInfoSubstate extends MusicBeatSubstate
{
	...
	public function new(){
		...
		for (i in 0...tipTextArray.length) {
			...
			FlxTween.tween(tipText, {y: (i * 12), alpha: 1}, 0.25, {ease: FlxEase.circOut, startDelay: 0.01 * i});			
		}
	}
	override public function update(elapsed:Float){
		...
		if(FlxG.keys.justPressed.ESCAPE){
			...
			FlxTween.tween(ChartingState.textBox, {x: 762.5, y: 433, alpha: 0.3}, 0.75, {ease: FlxEase.quartOut, startDelay: 0.25});
