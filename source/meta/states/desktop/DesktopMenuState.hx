...
class DesktopMenuState extends MusicBeatState
{
	...
	function lights()
	{
		...
		for (shits in [
			curtains, pizza, box, pc, lotion, wires2, mixer, bottle, phone, wires3, tissues, wires1, can1, can2, monitor, keyboard
		])
		{
		...
		if (fromMenu)
		{
			switch (whichMenu)
			{
				case "song", "options", "credits":
					for (i in entryGroup)
						i.visible = true;
					FlxTween.tween(FlxG.camera, {zoom: 0.6}, 3, {
	...
	override function update(elapsed:Float)
	{
		...
		else if (FlxG.mouse.overlaps(entryGroup[0]) || FlxG.mouse.overlaps(entryGroup[1]) || FlxG.mouse.overlaps(entryGroup[2])
			|| FlxG.mouse.overlaps(entryGroup[3]))
		{ // hhelp me
			if (lightSwitch)
				newCursor.mouseInterest = true;
		}
		...
		if (FlxG.mouse.justPressed && canSelect && !outOfRange)
		{
			...
			switch (menuItems[curSelected])
			{
				...
				case 'gallery':
					...
					FlxTween.tween(FlxG.camera, {zoom: 2.5}, time, {
						startDelay: 1,
						ease: FlxEase.cubeIn,
						onComplete: Void ->
						{
							...
							{
								MusicBeatState.switchState(new JermaState());
							});
						}
					});
					dimEverythingButTheMonitor();
	...
	{
		for (shits in [
			curtains, pizza, box, pc, lotion, wires2, mixer, bottle, phone, wires3, tissues, wires1, can1, can2, keyboard
		])
		{
			shits.alpha = 1;
