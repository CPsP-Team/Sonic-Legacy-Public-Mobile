    ...
    override function update(elapsed:Float) {
        ...
        if (!inSubMenu) 
        {
           ...
           // trace(_imSorryTmr);
            ...
            if (controls.BACK #if android || FlxG.android.justReleased.BACK #end && !introControlLock) {
...
class BookSubstate extends MusicBeatSubstate {
    ...
    override public function update(elapsed:Float){
        if((controls.BACK #if android || FlxG.android.justReleased.BACK #end || FlxG.mouse.justPressed) && canSelect && book.getCurAnimName() == 'open'){
            canSelect = false;
            book.y+=20;
