...
class DesktopOptionsState extends MusicBeatState
{
	...
	var mainOptionsList:Array<Array<String>> = [
		["NOTES", "Change the colors of your notes ingame."],
		...
		[
			"GAMEPLAY",
			"Change specific gameplay prefences used while playing through the PC."
		],
	...
	override function create()
	{
		...
		for (i in 1...5)
			soundz.push(FlxG.sound.load(Paths.sound('keyboard/click$i')));
			...
			new FlxTimer().start(0.25, function(t1:FlxTimer)
			{
				...
				new FlxTimer().start(0.06125, function(t2:FlxTimer)
				{
					...
					new FlxTimer().start(0.06125, function(t3:FlxTimer)
					{
						...
						FlxTween.tween(cmd.scale, {x: 1, y: 1}, 0.25, {
	...
	function selectOption(option:Int, list:Dynamic)
	{
		if (inMain)
		{
			...
			for (i in 0...mainOptionsGrp.length)
				mainOptionsGrp[i].visible = false;
					...
					switch (mainOptionsList[option][0])
					{
						...
						case "CONTROLS":
							...
							for (camera in [display, camCRT, FlxG.camera])
							{
								...
								FlxTween.tween(camera, {zoom: 4}, 1, {
									ease: FlxEase.cubeIn,
									onComplete: Void ->
									{
