...
class DesktopSongSelectState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{ // also later we use the uhhh unlocked songs to go filter out
	...
	{
		var up:Bool = true;
		if (mouseHitbox.y >= 581)
			up = false;
		var animLength = up ? 9 : 6;
		bf.animation.play(up ? 'up' : 'down', true, false,
			Math.floor(FlxMath.bound(FlxMath.remapToRange(mouseHitbox.x, 0, 882, 0, animLength), 0, animLength)));
	}
	...
	function diffSelectStuff()
	{
		...
		if (mouseHitbox.overlaps(diffOptions, display))
			newCursor.mouseInterest = true;
		else
			newCursor.mouseInterest = false;
	...
	function songSelectStuff()
	{
		...
		for (i in selectableSongs)
		{
			if (!choosingDiff && canSelect)
			{
				...
				{
					curSelected = i.ID;
					if (!StringTools.contains(i.animation.curAnim.name, 'hover'))
					{
						FlxG.sound.play(Paths.sound('SEL_misc2'));
						i.animation.play('hover');
					}
					if (FlxG.mouse.justPressed)
					{
			...
			// i.updateHitbox();
			if (i.animation.curAnim.name != 'hover-loop')
				i.centerOffsets(); // need to fix em anyway
		}
	...
	function loadSong(curSelected:Int, anim:FlxSprite)
	{
		...
		new FlxTimer().start(2, function(us:FlxTimer)
		{
			camCRT.fade(0xFF000000, 1, false, () ->
			{
				...
				if (song == 'obituary' && ClientPrefs.getGameplaySetting('botplay', false))
					song = Paths.formatToSongPath('free-4-me');
	...
	function spawnDiffSelector()
	{
		...
		var offsets:Array<Array<Float>> = [[59.55, 58.8], [264.5, 58.7], [475.8, 58.1]];
		...
		for (i in [diffBG, diffOptions])
			insert(members.indexOf(mouseHitbox), i);
