...
class DesktopCreditsState extends MusicBeatState
{
	...
	function monitorStuff()
	{
		...
		final offsets:Array<Array<Float>> = [[0, 0], [0, -3], [0, 3], [13, 0]];
			...
			new FlxTimer().start(0.5, function(t0:FlxTimer)
			{
				...
				new FlxTimer().start(0.06125, function(t2:FlxTimer)
				{
					...
					new FlxTimer().start(0.06125, function(t3:FlxTimer)
					{
						...
						FlxTween.tween(creditspadDummy.scale, {x: 1, y: 1}, 0.25, {
	...
	public var bfAnims:Array<String> = [
		"singUP",
		"singLEFT",
		"singDOWN",
		"singRIGHT",
	...
	function whoIsThisKid(id:Int)
	{
		...
		if (evilMode)
		{ // heeyyy... why does she get her OWN mode, and i don't! grrrrr......... -paige
			...
			var quote3 = new FlxText(75, contributions.y + contributions.height + 20, fieldWidth,
				"and even that evil fuck plankton. I also would like to thank Marco Antonio for making Mario's Madness. FNF Classified releases tomorrow, so make sure you check it out!");
	...
	override function update(elapsed:Float)
	{
		...
		if (!buddyListMode && mouseHitbox.overlaps(returnbutton, windowView) && FlxG.mouse.justReleased && !mouseDrag)
			gobackpls();

		if (FlxG.mouse.overlaps(windowBTNx, windowOverlay) && FlxG.mouse.justReleased && !mouseDrag)
			gobackpls(true);
		...
		scrollbarMid.y = FlxMath.bound(FlxMath.remapToRange(windowView.scroll.y, 0, scrollYmax, scrollBTNup.y
			+ scrollBTNup.height
			+ 5
			+ scrollbarTop.height,
			scrollBTNdown.y
			- scrollbarMid.height
			- 5
			- scrollbarBottom.height),
			scrollBTNup.y
			+ scrollBTNup.height
			+ 5
			+ scrollbarTop.height, scrollBTNdown.y
			- scrollbarMid.height
			- 5
			- scrollbarBottom.height);
		...
		if (FlxG.mouse.overlaps(windowBTNx, windowOverlay) && !mouseDrag)
		{
			if (FlxG.mouse.pressed)
				windowBTNx.animation.play("click");
			else
				windowBTNx.animation.play("idle");
		}
		else
		...
		if (FlxG.mouse.overlaps(windowBTNdisabled, windowOverlay) && !mouseDrag)
			newCursor.mouseDisabled = true;
		else
			newCursor.mouseDisabled = false;

		var mouseScreenPos = FlxG.mouse.getScreenPosition();
		var creditDummyPos = creditspadDummy.getScreenPosition();
		if (FlxMath.inBounds(mouseScreenPos.x, creditDummyPos.x, creditDummyPos.x + creditspadDummy.width)
			&& FlxMath.inBounds(mouseScreenPos.y, creditDummyPos.y + 75, creditDummyPos.y + creditspadDummy.height - 10))
		{
		...
		for (guh in [scrollBTNdown, scrollBTNup])
		{
			...
			{
				if (FlxG.mouse.pressed)
					guh.animation.play("click");
				else
					guh.animation.play("idle");
			}
			else
				guh.animation.play("idle");
		}
		...
		if (!buddyListMode && !mouseDrag)
		{
			...
			{
				if (FlxG.mouse.pressed)
					returnbutton.animation.play("click");
				else
					returnbutton.animation.play("idle");
			}
			else
		...
		{
			if (buddyListMode)
				curBuddyScrollY = FlxMath.bound(buddyScrollY += scrollbarDrag, 0, scrollYmax);
			else
				curKidScrollY = FlxMath.bound(kidScrollY += scrollbarDrag, 0, scrollYmax);
			if (FlxG.mouse.justReleased)
				mouseDrag = false;
		}
		...
		if (buddyListMode)
		{
			if (buddyScrollY <= 0)
				buddyScrollY = 0;
			if (buddyScrollY >= scrollYmax)
				buddyScrollY = scrollYmax;
			buddyScrollY -= buddyListMode ? scrollY : 0;
			if (scrollBTNdown.animation.curAnim.name == "click")
				buddyScrollY += 5;
			if (scrollBTNup.animation.curAnim.name == "click")
				buddyScrollY -= 5;
		}
		else
		{
			if (kidScrollY <= 0)
				kidScrollY = 0;
			if (kidScrollY >= scrollYmax)
				kidScrollY = scrollYmax;
		...
		if (boyfriendMode)
		{
			if (bf.animation.curAnim.curFrame >= 4)
				bf.playAnim(bfAnims[FlxG.random.int(0, bfAnims.length - 1)], true, false, 0);
			bf.flipX = false;
		}
	}
}
